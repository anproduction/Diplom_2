...
def test_create_order_authorized_with_ingredients(registered_user):
...
def test_create_order_without_ingredients(registered_user):
    login_response = requests.post(
        LOGIN,
        json={"email": registered_user["email"], "password": registered_user["password"]}
    )
...
def test_create_order_with_invalid_ingredients(registered_user):
    login_response = requests.post(
        LOGIN,
        json={"email": registered_user["email"], "password": registered_user["password"]}
    )
