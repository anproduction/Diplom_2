import requests
import allure
from endpoints import ORDER_CREATE, LOGIN
from data import VALID_INGREDIENTS, INVALID_INGREDIENTS


@allure.feature("Создание заказа")
class TestOrder:

    @allure.title("Создание заказа с авторизацией и ингредиентами")
    def test_create_order_authorized_with_ingredients(self, registered_user):
        with allure.step("Авторизация пользователя"):
            login_response = requests.post(
                LOGIN,
                json={"email": registered_user["email"], "password": registered_user["password"]}
            )
            assert login_response.status_code == 200
            token = login_response.json().get("accessToken")
            assert token is not None

        with allure.step("Формирование тела заказа и заголовков"):
            headers = {"Authorization": token}
            order_data = {"ingredients": VALID_INGREDIENTS}

        with allure.step(f"Отправка POST-запроса на {ORDER_CREATE} с авторизацией"):
            response = requests.post(ORDER_CREATE, json=order_data, headers=headers)

        with allure.step("Проверка успешного создания заказа"):
            assert response.status_code == 200
            resp_json = response.json()
            assert resp_json.get("success") is True
            assert "order" in resp_json

    @allure.title("Создание заказа без авторизации")
    def test_create_order_without_authorization(self):
        with allure.step("Формирование тела заказа"):
            order_data = {"ingredients": VALID_INGREDIENTS}

        with allure.step(f"Отправка POST-запроса на {ORDER_CREATE} без авторизации"):
            response = requests.post(ORDER_CREATE, json=order_data)

        with allure.step("Проверка ответа при отсутствии авторизации"):
            assert response.status_code in (200, 401)
            resp_json = response.json()
            assert "success" in resp_json

    @allure.title("Создание заказа без ингредиентов")
    def test_create_order_without_ingredients(self, registered_user):
        with allure.step("Авторизация пользователя"):
            login_response = requests.post(
                LOGIN,
                json={"email": registered_user["email"], "password": registered_user["password"]}
            )
            token = login_response.json().get("accessToken")
            headers = {"Authorization": token}

        with allure.step("Формирование тела заказа без ингредиентов"):
            order_data = {"ingredients": []}

        with allure.step("Отправка POST-запроса на создание заказа без ингредиентов"):
            response = requests.post(ORDER_CREATE, json=order_data, headers=headers)

        with allure.step("Проверка отказа в создании заказа"):
            assert response.status_code in (400, 422)
            resp_json = response.json()
            assert resp_json.get("success") is False

    @allure.title("Создание заказа с неверным хешем ингредиентов")
    def test_create_order_with_invalid_ingredients(self, registered_user):
        with allure.step("Авторизация пользователя"):
            login_response = requests.post(
                LOGIN,
                json={"email": registered_user["email"], "password": registered_user["password"]}
            )
            token = login_response.json().get("accessToken")
            headers = {"Authorization": token}

        with allure.step("Формирование тела заказа с невалидными ингредиентами"):
            order_data = {"ingredients": INVALID_INGREDIENTS}

        with allure.step("Отправка POST-запроса с невалидными ингредиентами"):
            response = requests.post(ORDER_CREATE, json=order_data, headers=headers)

        with allure.step("Проверка отказа в создании заказа"):
            assert response.status_code in (400, 500)
            resp_json = response.json()
            assert resp_json.get("success") is False
